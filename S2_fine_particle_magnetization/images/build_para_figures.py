"""Build the Langevin-function and Curie-law figures for the S2 deck.

Generates two PNGs in this directory, styled to match the deck (UMN maroon
accent, clean sans-serif axes), replacing the two-panel chapter figure
(para.png) with single-concept figures:

- langevin.png  — the Langevin function L(a) = coth(a) - 1/a with the
  low-field a/3 tangent, the saturation asymptote, and the 90%-saturation
  marker at a ~ 10
- curie_law.png — Curie's law chi_p ∝ 1/T, normalized to room temperature

Physics follows chapter 3 of Tauxe & Swanson-Hysell (2026), Essentials of
Paleomagnetism (Langevin 1905 theory).

Run in the ess-jbook environment:

    source ~/miniforge3/bin/activate ess-jbook
    python build_para_figures.py
"""

import matplotlib.pyplot as plt
import numpy as np

MAROON = '#7a0019'   # deck accent
GOLD = '#b8860b'     # deck highlight (darkened UMN gold for print contrast)
MUTED = '#555555'
GRID = '#dddddd'
...
def build_langevin():
    ...
    fig.savefig('langevin.png')
    plt.close(fig)
    print('wrote langevin.png')
...
def build_curie():
    ...
    fig.savefig('curie_law.png')
    plt.close(fig)
    print('wrote curie_law.png')
